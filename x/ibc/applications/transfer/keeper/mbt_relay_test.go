...
func AddressFromTla(addr []string) string {
	...
	if len(addr[0]) == 0 && len(addr[1]) == 0 {
		...
		s = fmt.Sprintf("%s\x00%s/%s", transfettypes.Version, addr[0], addr[1])
	} else {
...
func BankOfChain(chain *ibctesting.TestChain) Bank {
	...
	chain.GetSimApp().BankKeeper.IterateAllBalances(chain.GetContext(), func(address sdk.AccAddress, coin sdk.Coin) (stop bool) {
		...
		if strings.HasPrefix(coin.Denom, "ibc/") {
			fullDenom, _ = chain.GetSimApp().IBCTransferKeeper.DenomPathFromHash(chain.GetContext(), coin.Denom)
		}
...
func (suite *KeeperTestSuite) TestModelBasedRelay() {
	...
	for _, file_info := range files {
		...
		for i, tlaTc := range tlaTestCases {
			...
			registerDenom := func() {
				denomTrace := transfettypes.ParseDenomTrace(tc.packet.Data.Denom)
			...
			suite.Run(fmt.Sprintf("Case %s", description), func() {
				...
				case "FxSendTransfer":
					...
					if err == nil {
						...
						fee, ok := sdk.NewIntFromString(tc.packet.Data.Fee)
						if !ok {
							panic("MBT failed to parse amount from string")
						}
						err = suite.chainB.GetSimApp().TransferKeeper.FxSendTransfer(
							...
							0, tc.packet.Data.Router, sdk.NewCoin(denom, fee))
					}
				case "SendTransfer":
					var sender sdk.AccAddress
					sender, err = sdk.AccAddressFromBech32(tc.packet.Data.Sender)
					...
					if err == nil {
						...
						err = suite.chainB.GetSimApp().IBCTransferKeeper.SendTransfer(
							suite.chainB.GetContext(),
							tc.packet.SourcePort,
							tc.packet.SourceChannel,
							sdk.NewCoin(denom, amount),
				...
				case "OnRecvPacket":
					if len(tc.packet.Data.GetFee()) == 0 {
						tc.packet.Data.Fee = sdk.ZeroInt().String()
					}
					err = suite.chainB.GetSimApp().TransferKeeper.FxOnRecvPacket(suite.chainB.GetContext(), packet, tc.packet.Data)
				case "FxOnRecvPacket":
					if len(tc.packet.Data.GetFee()) == 0 {
						tc.packet.Data.Fee = sdk.ZeroInt().String()
					}
